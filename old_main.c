...
#define NULLSPACE_EXTRA 0
#define NULLSPACE_RADIUS 15
         ...
         if (frame > nullframe_begin && frame < nullframe_end) {
            ...
            for (int i=0; i<bufferinfo.length; i++) {
              ...
              if (dis_val < lower_null_frame[i]) {
                ...
                if (low_val > NULLSPACE_EXTRA) {
                  low_val -= NULLSPACE_EXTRA;
                }
                lower_null_frame[i] = low_val;
                if (upper_null_frame[i] - lower_null_frame[i] > NULLSPACE_RADIUS) {
                  upper_null_frame[i] = lower_null_frame[i] + NULLSPACE_RADIUS;
                }
              }
              if (dis_val > upper_null_frame[i]) {
                char high_val = dis_val;
                if (high_val < 128-NULLSPACE_EXTRA) {
                  high_val += NULLSPACE_EXTRA;
                }
                upper_null_frame[i] = high_val;
                if (upper_null_frame[i] - lower_null_frame[i] < NULLSPACE_RADIUS) {
                  lower_null_frame[i] = upper_null_frame[i] - NULLSPACE_RADIUS;
                }
              }
            }
